...
def constructBayesNet(gameState):
    ...
    for i in obsVars:
        variableDomainsDict[i] = OBS_VALS
...
def fillObsCPT(bayesNet, gameState):
    ...
    "*** YOUR CODE HERE ***"
    Position = {BOTTOM_LEFT_VAL: bottomLeftPos, TOP_LEFT_VAL: topLeftPos, BOTTOM_RIGHT_VAL: bottomRightPos, TOP_RIGHT_VAL: topRightPos}
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            obsVar = OBS_VAR_TEMPLATE % obsPos
            obsFac = bn.Factor([obsVar], HOUSE_VARS, bayesNet.variableDomainsDict())

            for assignment in obsFac.getAllPossibleAssignmentDicts():
                FoodPosi = assignment[FOOD_HOUSE_VAR]
                GhostPosi = assignment[GHOST_HOUSE_VAR]
                Color = assignment[obsVar]
                if Position[FoodPosi] == housePos or Position[GhostPosi] == housePos:
                    if Color == RED_OBS_VAL and Position[FoodPosi] == housePos:
                        prob = PROB_FOOD_RED
                    elif Color == RED_OBS_VAL and Position[GhostPosi] == housePos:
                        prob = PROB_GHOST_RED
                    elif Color == BLUE_OBS_VAL and Position[FoodPosi] == housePos:
                        prob = 1 - PROB_FOOD_RED
                    elif Color == BLUE_OBS_VAL and Position[GhostPosi] == housePos:
                    ...
                    if assignment[obsVar] == RED_OBS_VAL:
                        ...
                        prob = 1  
                obsFac.setProbability(assignment, prob)
            bayesNet.setCPT(obsVar, obsFac)
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    probs = [FoodHouseFactor.getProbability(assignment) for assignment in FoodHouseFactor.getAllPossibleAssignmentDicts()]
    assignments = [assignment for assignment in FoodHouseFactor.getAllPossibleAssignmentDicts()]
    highest_prob_index = probs.index(max(probs))
    optimal_assignment = assignments[highest_prob_index]
